...
[{ "name": "a", "value": 12, "color": "#4285F4" }] for Recharts API. Make sure field name always stays named name. Instead of naming value field value in JSON, name it based on user metric.\n Make sure the format use double quotes and property names are string literals. \n\n${inputValue}\n Provide JSON data only. `;
  ...
  return (
    <div className="flex flex-col px-4 items-center justify-center bg-black overflow-y-hidden">
      <Header />
      <Head>
        <title>AI tool to convert text to a beautiful chart</title>
      </Head>
      <div className="w-full max-w-xl mb-6 space-6 bg-black">
        <form
          onSubmit={handleSubmit}
          className="w-full max-w-xl mb-1 bg-black rounded-lg"
        >
          <div className="flex flex-col items-center justify-center rounded-lg">
            <label
              htmlFor="textInput"
              className="block mt-5 font-sans font-semibold text-white"
            >
            ...
            <textarea
              ...
              rows={3}
              placeholder=""
              className="bg-zinc-800 appearance-none font-inter mt-8 border-white/20 shadow-sm flex flex-col items-center justify-center rounded-lg w-full max-w-md py-2 px-3  text-gray-200 leading-tight focus:outline-none focus:shadow-outline text-left min-h-[120px] max-h-[200px]"
              value={inputValue}
              required
              autoFocus
      ...
      <h1 className="text-4xl mb-6 xl:text-4xl font-extrabold text-center xl:text-left text-white">
        Get a new chart in a few steps
      </h1>
      <InfoSection />
      <footer className="text-center font-sans text-white text-ml mb-10">
